...
const CODE_EXTENSIONS = new Set([
  ...
  "ts",
  "html",
  // Removido "css" de las extensiones permitidas
  "java",
  "cpp",
  "cs",
]);
...
const CONFIG_PATTERNS = [
  ...
  /(^|\/)(config|\.github|\.vscode|\.idea|tests?|__mocks__|docker|scripts)(\/|$)/i,

  // Extensiones de archivos de configuración (agregado css)
  /\.(json|config|yml|yaml|toml|xml|env|ini|cfg|conf|dockerfile|makefile|bak|backup|css)$/i,

  // Archivos específicos de Tailwind y configuraciones
  /(tailwind|postcss)\.config\.[jt]s$/i,
  /tailwind\.css$/i,

  // Nombres de archivos específicos
  /^(package\.json|tsconfig\.json|webpack\.config\.js|jest\.config\.js|\.babelrc|\.eslintrc|\.prettierrc|docker-compose\.yml)/i,
];
...
export default function FlowSidebar({ apiUrl, repoData }: FlowSidebarProps) {
  ...
  const filterCodeFiles = useCallback((files: RepoItem[]) => {
    return files.filter((file) => {
      ...
      // Verificar si es archivo de código válido
      const isCodeFile = CODE_EXTENSIONS.has(extension);

      // Verificar si es archivo de configuración
      const isConfigFile = CONFIG_PATTERNS.some(
        (pattern) =>
          pattern.test(fullPath) || pattern.test(file.name.toLowerCase())
      );

      return isCodeFile && !isConfigFile;
    });
  }, []);
